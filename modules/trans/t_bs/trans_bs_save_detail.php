	...
	*/
	...
	if ($_GET[add_detail]=='true') {
		...
		001	Bank (K)
		 ...
		 ======Jurnal Penyelesaian BS (PB)======
		000	BS (K)
		001	Beban(D)
		002 Bank (D) / Kelebihan BS dikembalikan

		000	BS (K)
		001	Beban(D)
		002 Bank (K) / Kekurangan BS 
		*/
		...
		if ($macc==$_POST[txt_d_no_akun]) {
			...
			$ket=str_replace("'","\'",$_POST[txt_d_nmket]);
			$ket=str_replace('"',"\'",$ket);
			...
			$qry_ins = mysql_query($qry, $conn) or die("Error Insert Detail ".mysql_error());
