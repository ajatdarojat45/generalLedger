...
function doAdd()
{
	...
	acc=window.localStorage.getItem('_acc3');
	nmp=window.localStorage.getItem('_nmp3');
	...
	editmodeinduk='NEW';
	url="trans_crud.php?trx=BS&crud=NEW";	
}
// --------------------------------------------------------------------------------------------
function doEdit()
{	
	...
	editmodeinduk='EDIT';
	url="trans_crud.php?trx=BS&crud=EDIT";	
	//refreshDetail()	;
...
{	
	...
	$.messager.confirm('Delete','Hapus Transaksi Jurnal '+nbk+' ?',function(r){
		if (r) {			
			$.post('trans_crud.php?trx=BS&crud=DELETE',{nbk:nbk},function(result){
				if (result.success){
...
function doPrint()
{
	...
	document.getElementById('frame_pdf').src="cetak_jurnal.php?tipe=S&nbk="+nbk;			
