...
if (isset($_SESSION['app_glt'])) {	
	...
	*/	
	...
	if ($_GET[edit_detail]=='true') {
		...
		$totaldebet=$data_total[total_debet];
		$totalkredit=$data_total[total_kredit];

		// update nilai jumlah akun pusatnya
		if ($_GET[tipe]=='K') {
			// transaksi kas keluar
			$qry = "UPDATE $tbl_trx_temp SET krd=$totaldebet,deb=0, pemakai='$user_id' WHERE seq='000' AND user_id='$user_id' AND mcom_id='$company_id' AND nbk= '$_GET[no_bukti]'";
			$qry_ins = mysql_query($qry, $conn) or die("Update Temp Caseglt 000".mysql_error());
		} else {
			//transaksi kas masuk
			$qry = "UPDATE $tbl_trx_temp SET deb=$totalkredit,krd=0, pemakai='$user_id' WHERE seq='000' AND user_id='$user_id' AND mcom_id='$company_id' AND nbk= '$_GET[no_bukti]' ";
			$qry_ins = mysql_query($qry, $conn) or die("Update Temp Caseglt 000".mysql_error());
		}	
	}
	...
	while ($data = mysql_fetch_array($qry_tmp)) {
		$no ++;
		if ($tmbl_edit==2) {
			$view_tmbl_edit="<button type='button' class='btn-link' name=\"btn_edit_detail\" id=\"edit_$data[seq]\"  data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Edit data $data[seq]\" onclick=\"klik_d_edit('$data[seq]','$data[acc]','$data[nmp]','$data[ket]','$data[kd_sup]','$data[nm_sup]','$data[deb]','$data[krd]')\" ><span class=\"glyphicon glyphicon-edit\"/></span></button>";
