	...
	*/
	...
	if ($_GET[add_detail]=='true') {
		...
		if ($seq=="001") {
			...
			// Kode akun pusat
			$qry ="INSERT INTO $tbl_trx_temp (mcom_id, trx_status, nbk, tgp, seq, typ, acc, deb, krd, ket, flg, pemakai, tgl_input, user_id) VALUES ('$company_id', '1', '$_GET[no_bukti]', '$tgp', '000', '$tipe_trans', '$_POST[txt_noakun]', '$jml_debet', '$jml_kredit', '$_POST[txt_ket]', '0', '$user_id', now(), '$user_id') ";
		...
		if ($data[acc]==$_POST[txt_d_no_akun]) {
			...
			$qry ="INSERT INTO $tbl_trx_temp (mcom_id, trx_status, nbk, tgp, seq, typ, acc, deb, krd, ket, flg, kd_sup, pemakai, tgl_input, user_id) VALUES ('$company_id', '1', '$_GET[no_bukti]', '$tgp', '$seq', '$tipe_trans', '$_POST[txt_d_no_akun]', $jml_d_debet, $jml_d_kredit, '$_POST[txt_d_nm_ket]', '0','$_POST[txt_d_no_sup]', '$user_id', now(), '$user_id') ";		
			...
			if ($seq<>'001'){
				...
				if ($_GET[tipe]=='K') {
					...
					$qry = "UPDATE $tbl_trx_temp SET krd=krd+$_POST[txt_d_jml],pemakai='$user_id' WHERE seq='000' AND user_id='$user_id' AND mcom_id='$company_id' AND nbk= '$_GET[no_bukti]' ";
					$qry_ins = mysql_query($qry, $conn) or die("Update Temp Caseglt 000".mysql_error());
				} else {
					//transaksi kas masuk
					$qry = "UPDATE $tbl_trx_temp SET deb=deb+$_POST[txt_d_jml],pemakai='$user_id' WHERE seq='000' AND user_id='$user_id' AND mcom_id='$company_id' AND nbk= '$_GET[no_bukti]' ";
					$qry_ins = mysql_query($qry, $conn) or die("Update Temp Caseglt 000".mysql_error());
				}
			}
		}
	...
	if ($_GET[edit_detail]=='true') {
		...
		$qry ="UPDATE $tbl_trx_temp SET acc='$_POST[txt_d_no_akun]' , deb=$jml_d_debet, krd=$jml_d_kredit, ket='$_POST[txt_d_nm_ket]', kd_sup='$_POST[txt_d_no_sup]', pemakai='$user_id', tgl_input='$_POST[cdatetime]', user_id='$user_id' WHERE nbk='$_GET[no_bukti]' AND seq='$seq' AND mcom_id='$company_id' AND user_id='$user_id'";		
		...
		if ($_GET[tipe]=='K') {
			...
			// transaksi kas keluar
			$qry = "UPDATE $tbl_trx_temp SET krd=$data_total[total_debet],pemakai='$user_id' WHERE seq='000' AND user_id='$user_id' AND mcom_id='$company_id' AND nbk= '$_GET[no_bukti]'";
			$qry_ins = mysql_query($qry, $conn) or die("Update Temp Caseglt 000".mysql_error());
		} else {
			//transaksi kas masuk
			$qry = "UPDATE $tbl_trx_temp SET deb=$data_total[total_kredit],pemakai='$user_id' WHERE seq='000' AND user_id='$user_id' AND mcom_id='$company_id' AND nbk= '$_GET[no_bukti]' ";
			$qry_ins = mysql_query($qry, $conn) or die("Update Temp Caseglt 000".mysql_error());
		}	
	}
	...
	if ($_GET[delete_detail]=='true') {
		...
		if ($jmlrec[jml_rec]==1) {
			...
			if ($_GET[tipe]=='K') {
				...
				$data_total=mysql_fetch_array($qry_tmp);			
				// transaksi kas keluar
				$qry = "UPDATE $tbl_trx_temp SET krd=$data_total[total_debet] WHERE seq='000' AND user_id='$user_id' AND mcom_id='$company_id' AND nbk= '$_GET[no_bukti]'";
				$qry_ins = mysql_query($qry, $conn) or die("Update Temp Caseglt 000".mysql_error());
			} else {
				//transaksi kas masuk
				$qry = "UPDATE $tbl_trx_temp SET deb=$data_total[total_kredit] WHERE seq='000' AND user_id='$user_id' AND mcom_id='$company_id' AND nbk= '$_GET[no_bukti]' ";
				$qry_ins = mysql_query($qry, $conn) or die("Update Temp Caseglt 000".mysql_error());
			}
	...
	$qry_tmp = mysql_query("SELECT a.*,b.nmp as nmp FROM ".$tbl_trx_temp." as a left join ".$tbl_mst_akun." as b on a.acc=b.acc WHERE a.user_id='$user_id' AND a.trx_status='1' ORDER BY a.seq", $conn) or die("Error Select Show Detail ".mysql_error());
	$no=0;			
	while ($data = mysql_fetch_array($qry_tmp)) {
		$no ++;
		if ($tmbl_edit==2) {
			$view_tmbl_edit="<button type='button' class='btn-link' name=\"btn_edit_detail\" id=\"edit_$data[seq]\"  data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Edit data $data[seq]\" onclick=\"klik_d_edit('$data[seq]','$data[acc]','$data[nmp]','$data[ket]','$data[kd_sup]','$data[deb]','$data[krd]')\" ><span class=\"glyphicon glyphicon-edit\"/></span></button>";
