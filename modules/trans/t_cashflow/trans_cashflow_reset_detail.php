...
if (isset($_SESSION['app_glt'])) {	
	...
	if ($_GET[kode_reset]<>"New"){	
		...
		$que_view = mysql_query("SELECT * FROM $tbl_view_trx WHERE nbk='$nbk' ", $conn) or die("Error Query count_categ top");
		$data_kas = mysql_fetch_array($que_view);		
		...
		$qry_tmp1 = mysql_query("SELECT * FROM $tbl_trx_temp WHERE user_id='$user_id'", $conn) or die("Error Select Temp Caseglt ".mysql_error());		
		$data_hasil = mysql_fetch_array($qry_tmp1);

		$qry_tmp2 = mysql_query("SELECT a.acc as acc,b.nmp as nmp,c.nm_sup as nm_sup FROM $tbl_trx_temp as a left join $tbl_mst_akun as b on a.acc=b.acc left join $tbl_mst_sup as c on a.kd_sup=c.kd_sup WHERE a.user_id='$user_id' AND a.seq='000' ", $conn) or die("Error Select Temp Caseglt ".mysql_error());
	...
	$qry_tmp4 = mysql_query("SELECT a.*,b.nmp as nmp FROM $tbl_trx_temp as a left join $tbl_mst_akun as b on a.acc=b.acc WHERE a.user_id='$user_id' AND a.trx_status='1' ORDER BY a.seq", $conn) or die("Error Select Show Detail ".mysql_error());
	$no=0;			
